...
/**
 * MY_split_string - splits a string into words specified by a delimiter(s),
...
char **MY_split_string(char *str, char *separators, size_t *word_count)
{
	...
	MY_set_zeros(word_sizes, MAX_WORD_COUNT);
	no_of_words = MY_words_count(str, separators, word_sizes);
	...
	for (v = 0; v < no_of_words; v++)
	{
		...
		{
			while (MY_is_delimiter(*str_ptr, separators))
				str_ptr++;
...
char **MY_strtow(char *str, char *d)
{
	...
	if (!d)
		d = " ";
	for (i = 0; str[i] != '\0'; i++)
		if (!MY_is_delimiter(str[i], d) && (MY_is_delimiter(str[i + 1], d) || !str[i + 1]))
			numwords++;
	if (numwords == 0)
		return (NULL);
	...
	{
		while (MY_is_delimiter(str[i], d))
			i++;
		k = 0;
		while (!MY_is_delimiter(str[i + k], d) && str[i + k])
			k++;
